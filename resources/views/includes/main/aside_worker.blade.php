<aside id="aside_block" class="fixed overflow-y-auto top-0 left-0 z-20 w-[var(--width-aside)] h-[100vh] bg-white rounded-tr-lg shadow rounded-br-lg p-2
        ...
        <ul class="flex flex-col gap-y-4 text-center">
            <li>
                <a class="aside_point @activeMenu('*profile*')"
                    href="{{ route('worker.profile') }}">{{ __('app.profile') }}</a>
            </li>
            <li>
                <a class="aside_point @activeMenu('*notifications*')"
                    href="{{ route('worker.notifications') }}">{{ __('app.notifications') }}</a>
            </li>
            <li>
