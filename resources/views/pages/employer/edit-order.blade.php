...
@section('title', __('app.edit_order'))

@section('content')
    <h1 class="page_title">{{ __('app.edit_order') }}</h1>
    <section>
        @include('includes.components.session-success-or-error')
        <form action="{{ route('orders.update', $order->getKey()) }}" method="post" class="max-w-[600px]">
            @csrf
            @method("PUT")
            @if ($errors->any())
            ...
            @endif
                <div class="flex flex-col gap-8">
                        ...
                        <div class="flex items-start gap-4 mb-4">
                            @foreach (PaymentType::cases() as $key => $value)
                                <label class="cursor-pointer text-lg inline-flex items-center gap-2">
                                    {{ $value }}
                                    <input @checked($value->value === old('payment_type', $order->payment_type)) name="payment_type" value="{{ $value->value }}" onchange="update_visible_payments_type_more()" class="accent-black w-5 h-5 payment_type" type="radio" />
                                </label>
                            @endforeach
                        </div>
                        <div>
                                    ...
                                    <div>
                                        ...
                                        <select name="payment_per" class="select-default">
                                            @foreach ($payment_pers as $payment_per)
                                                <option @selected($payment_per->getKey() == old('payment_per') || $payment_per->name === $order->payment_per) value="{{ $payment_per->getKey() }}">{{ $payment_per->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            @foreach ($categories_works as $category_work)
                                <label class="py-1 px-3 bg-black text-white rounded-lg flex items-center gap-4 cursor-pointer min-h-11">
                                    ...
                                    <input class="accent-fox w-5 h-5" type="checkbox" name="categories_selected[]"
                                        value="{{ $category_work->id }}" @checked(in_array($category_work->id, old('categories_selected', $order->workCategories->pluck('id')->toArray()))) />
                                </label>
                            @endforeach
                        </div>
                    </div>
                    @include('includes.components.input', [
                        ...
                        'type' => "number",
                        'default_value' => $order->need_count_workers,
                    ])
                    <div>
                        ...
                        <div class="many_images_container" id="images_container">
                            @php
                                $images = old('images', $order->images);
                            @endphp
                            @if ($images)
                    ...
                    <button class="button-default !bg-fox">Внести изменения</button>
                </div>
            </form>
        </section>
@endsection
